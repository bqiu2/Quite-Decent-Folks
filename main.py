...
def _difficulty_picker() -> DifficultyConfig | None:
    ...
    try:
        while True:
            ...
            screen.fill((28, 52, 40))
            title = title_font.render("CHOOSE DIFFICULTY", True, (255, 222, 96))
            screen.blit(title, title.get_rect(center=(380, 75)))
            hint = body_font.render(
                "Use 1/2/3 or arrow keys, then press Enter", True, (246, 241, 211)
            )
            ...
            for index, name in enumerate(names):
                ...
                pygame.draw.rect(
                    screen,
                    (88, 151, 102) if active else (53, 88, 66),
                    rect,
                    border_radius=12,
                )
                pygame.draw.rect(
                    screen,
                    (255, 220, 92) if active else (105, 145, 111),
                    rect,
                    width=3,
                    border_radius=12,
                )
                label = title_font.render(name.upper(), True, (255, 250, 220))
...
def _show_final_result(result: FinalResult) -> None:
    ...
    while True:
        ...
        screen.fill((24, 43, 37))
        color = (255, 222, 96) if result.victory else (235, 104, 85)
        title = title_font.render(
            "VICTORY" if result.victory else "DEFEAT", True, color
        )
        ...
        for index, row in enumerate(rows):
            text = body_font.render(row, True, (246, 241, 211))
            screen.blit(text, text.get_rect(center=(400, 145 + index * 42)))
        final_text = title_font.render(f"FINAL SCORE  {result.final_score:.2f}", True, color)
        screen.blit(final_text, final_text.get_rect(center=(400, 410)))
        hint = small_font.render("Press Enter, Space or Escape to close", True, (181, 224, 205))
