...
def _show_analysis_preview(plant: PlantData) -> bool:
    ...
    try:
        while True:
            ...
            draw_pixel_backdrop(screen, base=(188, 216, 205), horizon=580)
            pygame.draw.rect(screen, (153, 190, 153), (0, 580, 960, 40))
            for x in range(0, 960, 48):
                pygame.draw.rect(screen, (111, 157, 112), (x + 8, 580, 22, 4))

            header = pygame.Rect(24, 20, 912, 76)
            draw_pixel_panel(
                screen,
                header,
            ...
            screen.blit(title, (52, 38))
            subtitle = body_font.render(
                f"ID  {plant.plant_id}    CLASS  {plant.plant_type.upper()}",
                False,
                PALETTE["cream"],
            )
            screen.blit(subtitle, (520, 47))
            draw_pixel_badge(screen, pygame.Rect(770, 25, 132, 26), "SCAN READY", fill=PALETTE["green"])

            left_card = pygame.Rect(35, 120, 430, 440)
            right_card = pygame.Rect(490, 120, 435, 440)
            ...
            screen.blit(left_title, (62, 148))
            pygame.draw.rect(screen, (101, 125, 94), (62, 180, 350, 2))
            draw_status_hexagon(
                screen,
                (250, 300),
                100,
            ...
            draw_power_readout(screen, (82, 466), plant.initial_power, large=True)
            power_note = small_font.render(
                "Initial power = weakest nutrient x pest health",
                False,
                PALETTE["muted_cream"],
            )
            screen.blit(power_note, (82, 526))

            right_title = body_font.render("GENERATED PIXEL FORM", False, PALETTE["cream"])
            screen.blit(right_title, (520, 148))
            pygame.draw.rect(screen, (101, 125, 94), (520, 180, 350, 2))
            draw_pixel_plant(screen, (705, 338), plant.plant_type, scale=4)
            form_label = title_font.render(plant.plant_type.upper(), False, PALETTE["gold_light"])
            screen.blit(form_label, form_label.get_rect(center=(705, 450)))
            detail = small_font.render(
                "YOUR HERO FORM / READY FOR LEVEL 1",
                False,
                PALETTE["muted_cream"],
            )
            screen.blit(detail, detail.get_rect(center=(705, 482)))
            pygame.draw.rect(screen, (101, 125, 94), (520, 502, 350, 2))
            ...
            screen.blit(hint, hint.get_rect(center=(480, 598)))
            pygame.display.flip()
            clock.tick(30)
    finally:
        pygame.display.quit()
...
def _difficulty_picker() -> DifficultyConfig | None:
    ...
    try:
        while True:
            ...
            draw_pixel_backdrop(screen, base=(177, 210, 198), horizon=410)
            pygame.draw.rect(screen, (103, 157, 111), (0, 410, 760, 60))
            header = pygame.Rect(24, 20, 712, 118)
            ...
            screen.blit(title, title.get_rect(center=(380, 75)))
            hint = body_font.render(
                "Use 1/2/3 or arrow keys, then press Enter", False, PALETTE["cream"]
            )
            screen.blit(hint, hint.get_rect(center=(380, 125)))

            for index, name in enumerate(names):
                rect = pygame.Rect(75 + index * 230, 190, 190, 150)
                ...
                screen.blit(label, label.get_rect(center=(rect.centerx, rect.top + 48)))
                ...
                screen.blit(detail, detail.get_rect(center=(rect.centerx, rect.top + 105)))
                ...
                screen.blit(detail2, detail2.get_rect(center=(rect.centerx, rect.top + 130)))
...
def _show_final_result(result: FinalResult) -> None:
    ...
    while True:
        ...
        draw_pixel_backdrop(screen, base=(178, 208, 197), horizon=450)
        pygame.draw.rect(screen, (102, 151, 108), (0, 450, 800, 70))
        ...
        screen.blit(hint, hint.get_rect(center=(400, 470)))
        pygame.display.flip()
        clock.tick(30)
